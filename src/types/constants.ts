// constants.ts
import { AyahId, Category } from "./types";
...
// Provided AthmanList (assumed to be starting Ayah IDs for Rub el Hizb)
...
// Convert Hizb range to Rub el Hizb range (1 Hizb = 4 Rub el Hizb)
export const getRubRangeFromHizbRange = (minHizb: number, maxHizb: number): [number, number] => {
  const minRub = (minHizb - 1) * 4 + 1; // Starting Rub el Hizb
  const maxRub = maxHizb * 4; // Ending Rub el Hizb
  return [minRub, maxRub];
};

// Randomly select a Rub el Hizb number from the category range
export const getRandomRubNumber = (minRub: number, maxRub: number): number => {
  return Math.floor(Math.random() * (maxRub - minRub + 1)) + minRub;
};